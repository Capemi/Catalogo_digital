...
def conectar_bd():
    con = sqlite3.connect(f'{ruta_actual}/Catalogo_digital-main/Articuloss.db')
    return con
...
class Ventana_p:
    def __init__(self, root):
        ...
        # Construye la ruta al archivo en la carpeta "fotos"
        self.ruta_foto = os.path.join(self.ruta_actual, 'Catalogo_digital-main/fotos')

        def obtener_interna():
            # Configura el idioma local a español
            locale.setlocale(locale.LC_TIME, 'es_ES.UTF-8')

            with open(f"{self.ruta_foto}/verificacion.txt", "r") as file:
                for line in file:
        ...
        meta = Image.open(f"{self.ruta_foto}/meta.png")      
        auto = Image.open(f"{self.ruta_foto}/liviana.png")
        camion = Image.open(f"{self.ruta_foto}/pesada.png")      
        engra = Image.open(f"{self.ruta_foto}/OT.png")                     
        ...
        if monitor.width <= 1280:
            ...
            dev = Image.open(f"{self.ruta_foto}/dev.png").resize((30,20))

            lupa = Image.open(f"{self.ruta_foto}/lupa.png").resize((20, 20))
            eliminar = Image.open(f"{self.ruta_foto}/eliminar.png").resize((30, 30))
            back = [30,30]
            ...
            if monitor.width <=1440:
                ...
                ancho_ventana = int(monitor.width)
                alto_ventana = int(monitor.height)
                dev = Image.open(f"{self.ruta_foto}/dev.png").resize((40,30))

                lupa = Image.open(f"{self.ruta_foto}/lupa.png").resize((20, 20))
                eliminar = Image.open(f"{self.ruta_foto}/eliminar.png").resize((50, 50))
                back = [40,40]
                ...
                lupa = Image.open(f"{self.ruta_foto}/lupa.png").resize((50, 50))
                eliminar = Image.open(f"{self.ruta_foto}/eliminar.png").resize((50, 50))
                back = [40,40]
                ...
                alto_ventana = int(monitor.height)
                dev = Image.open(f"{self.ruta_foto}/dev.png").resize((40,30))
                lupa = Image.open(f"{self.ruta_foto}/lupa.png").resize((70, 70))
                back = [50,50] 
                eliminar = Image.open(f"{self.ruta_foto}/eliminar.png").resize((50, 50))
                ...
                lupa = Image.open(f"{self.ruta_foto}/lupa.png").resize((50, 50))
                back = [60,60]
                eliminar = Image.open(f"{self.ruta_foto}/eliminar.png").resize((60, 60)) 
        ...
        self.back =   CTkImage(Image.open(f"{self.ruta_foto}/back.png"), size=(back[0], back[1]))
        ...
        x = round((monitor.width - ancho_ventana)//2)
        y = round((monitor.height - alto_ventana) //2)
        
        self.root.title("CATÁLOGO DIGITAL - CAPEMI® A.Giacomelli S.A.")
        self.root.iconbitmap(f"{self.ruta_foto}/icono.ico")
        
        self.root.geometry(f"{ancho_ventana+5}x{alto_ventana}+{x}+{y}")

        self.root.minsize(ancho_ventana, alto_ventana-60)
        self.root.maxsize(ancho_ventana+5, alto_ventana-60)
        ...
        self.frame_botones.place(relx=0.69, rely=0.08, relheight=0.1)

        def enter_botones(event):
            if event.widget.cget("text") =="|" or event.widget.cget("text") =="Verificar actualizaciones": 
                pass
            else:
                event.widget.configure(font=("SEGOE UI",  self.letra+5, "bold"), justify="right", anchor="e")
                event.widget.configure(foreground='dodgerblue3')

            if event.widget.cget("text") == "Lanzamientos":
                boton1.config(font=("SEGOE UI",  self.letra+5, "bold"))           
            elif event.widget.cget("text") == "Contacto":
                
                boton2.config(font=("SEGOE UI",  self.letra+5, "bold"))
               
            elif event.widget.cget("text") == "Empresa":
                boton3.config(font=("SEGOE UI",  self.letra+5, "bold"))
                
            elif event.widget.cget("text") =="|":
                boton3.config(font=("SEGOE UI",  self.letra+5, "bold"), foreground='dodgerblue3', justify="right", anchor="e")

            elif event.widget.cget("text") == "Verificar actualizaciones":
                event.widget.config(foreground='dodgerblue3')

        
        def salir_botones(event):

            boton1.config(font=("SEGOE UI",  self.letra+3, "bold"), foreground='#04366D')
            boton2.config(font=("SEGOE UI",  self.letra+3, "bold"), foreground='#04366D')
            boton3.config(font=("SEGOE UI",  self.letra+3, "bold"), foreground='#04366D')
            boton_verificar.config(foreground='#04366D')
            
        boton1 = tk.Button(self.frame_botones, text = "Lanzamientos" ,fg="#04366D", borderwidth=0, highlightthickness=0, cursor="hand2",font=("SEGOE UI",  self.letra+3, "bold"), bg= "#ededed", anchor="center",justify="center", activebackground="#ededed", activeforeground= "dodgerblue3")
        boton1.bind("<Enter>",enter_botones)
        boton1.bind("<Leave>", salir_botones)
        boton2 = tk.Button(self.frame_botones, text = "Contacto" ,fg="#04366D", borderwidth=0, highlightthickness=0, cursor="hand2",font=("SEGOE UI",  self.letra+3, "bold"), bg= "#ededed", justify="right", anchor="e", command =self.contacto, activebackground="#ededed", activeforeground= "dodgerblue3")
        boton2.bind("<Enter>",enter_botones)
        boton2.bind("<Leave>", salir_botones)
        boton3 = tk.Button(self.frame_botones, text = "Empresa" ,fg="#04366D", borderwidth=0, highlightthickness=0, cursor="hand2",font=("SEGOE UI",  self.letra+3, "bold"), bg= "#ededed", justify="right", anchor="e", command=self.empresa, activebackground="#ededed", activeforeground= "dodgerblue3")
        boton3.bind("<Enter>",enter_botones)
        boton3.bind("<Leave>", salir_botones)
        '''
        ...
        self.frame_botones.columnconfigure(5)
        ...
        barra2.bind("<Enter>",enter_botones)
        barra2.bind("<Leave>", salir_botones)
        
        #barra3 = tk.Label(self.frame_botones, text="|", fg="#04366D",font=("SEGOE UI",  self.letra+20, "bold")) 

        boton1.grid(column=0, row=0, pady=15)
        barra1.grid(column=1, row=0, pady=15)
        boton2.grid(column=2, row=0, pady=15)
        barra2.grid(column=3, row=0, pady=15)
        boton3.grid(column=4, row=0, pady=15)
        #barra3.grid(column=5, row=0)
        #boton4.grid(column=6, row=0)
        
        label_actualizado = tk.Label(self.root, text = f"Actualizado el: {obtener_interna()}", font= ("SEGOE UI", self.diez, "bold"), fg= "#04366D", bg="white")
        boton_verificar = tk.Button(self.root, text = "Verificar actualizaciones", font= ("SEGOE UI", self.diez, "bold"), fg= "#04366D", bg="white", borderwidth=0, highlightthickness=0, cursor="hand2" , activebackground= "white", activeforeground=  "dodgerblue3", command= lambda: self.circulo_carga(ancho_ventana,alto_ventana))
        boton_verificar.bind("<Enter>",enter_botones)
        boton_verificar.bind("<Leave>", salir_botones)
        self.boton_dev = tk.Button(self.root,image=self.dev,bg="white", borderwidth=0, highlightthickness=0, cursor="hand2",activebackground= "white")
        label_actualizado.place(relx=0.1, rely=0)
        boton_verificar.place(relx=0.7, rely=0)     
        self.boton_dev.place(relx=0.96, rely=0)
    ...
    def verificar_si_actualizacion(self):
        ...
        def nueva_fecha_interna(fecha):
            with open('Catalogo_digital-main/fotos/verificacion.txt', 'w') as archivo:
        ...
        try:
            ...
            c= CTkMessagebox(title="Info", message="NO HAY ACTUALIZACIONES DISPONIBLES.\nDisfrute su ultima version",icon=f"{self.ruta_foto}/ele_blanco.png", option_1="Gracias", option_focus=1, button_color="#ededed", button_hover_color="dodgerblue3", fg_color="#032956", text_color="white", border_color="dodgerblue3", bg_color="#04366D", title_color="white", button_text_color="#032956", font=("SEGOE UI", 15, "bold"))   
            c.focus_set()            
            c.bind("<Return>", eliminar)
            return
        ...
        if diferencia_db.total_seconds() < 0:           
            ...
            self.popup.destroy()
            ruta_del_archivo = "Catalogo_digital-main/catalogo_a.py" 

            if os.path.exists(ruta_del_archivo):
                c= CTkMessagebox(title="Info", message="Es necesario reiniciar para cargar la actualización\n¿Desea hacerlo ahora?",icon=f"{self.ruta_foto}/ele_blanco.png", option_1="SI",option_2="No, gracias", option_focus=1, button_color="#ededed", button_hover_color="dodgerblue3", fg_color="#032956", text_color="white", border_color="dodgerblue3", bg_color="#04366D", title_color="white", button_text_color="#032956", font=("SEGOE UI", 15, "bold"))   
                c.focus_set()            
                
                g = c.get()
                if g == "SI":
                    from transition import TransitionFrame
                    import main
                    self.root.destroy()
                    next_window = tk.Toplevel()       
                    second_interface = main.Iniciar(next_window)

                    
                    transition_frame = TransitionFrame(self.root,next_window)
                    transition_frame.pack(fill=tk.BOTH, expand=True)
                    transition_frame.start_transition()
                ...
                image = Image.open(f"{self.ruta_foto}/banner.png").resize((ancho,alto), Image.BILINEAR)       
                imagen_tk = ImageTk.PhotoImage(image)
                self.lab.configure(image=imagen_tk)
                self.lab.image = imagen_tk

                c= CTkMessagebox(title="Info", message="¡Catálogo actualizado con exito!",icon=f"{self.ruta_foto}/ele_blanco.png", option_1="Gracias", option_focus=1, button_color="#ededed", button_hover_color="dodgerblue3", fg_color="#032956", text_color="white", border_color="dodgerblue3", bg_color="#04366D", title_color="white", button_text_color="#032956", font=("SEGOE UI", 15, "bold"))   
                c.focus_set()            
                c.bind("<Return>", eliminar) 
    ...
    def circulo_carga(self, ancho,alto):
        ...
        def iniciar_descarga():
            ...
            self.popup = tk.Toplevel(self.root, background="white")
            ...
            imagenn = CTkImage(Image.open(f"{self.ruta_foto}/capemi.png"), size=(383, 154))

            frame = ctk.CTkLabel(self.popup, image=imagenn, text="", fg_color="white")
            frame.place(relx=0.5,rely=0.2,anchor="center")
            lab = tk.Label(self.popup, text="¡Espere unos segundos por favor!\nDescargando actualizaciones", font=("SEGOE UI", self.letra+10, "bold"), fg="#04366D", bg="white")
    ...
    def karim(self):
        top = tk.Toplevel(bg="gray9")
        
        # Calcular la posición para centrar la ventana TopLevel
        x = self.root.winfo_screenwidth() // 2 - 600 // 2
        y = self.root.winfo_screenheight() // 2 - 300 // 2
        top.geometry(f"600x300+{x}+{y-20}")
        top.resizable(0,0)
        top.iconbitmap(f"{self.ruta_foto}/python.ico")
        top.title("Desarrollador:")
        imagenn = CTkImage(Image.open(f"{self.ruta_foto}/karim_banner.png"), size=(500, 150))
        b = ctk.CTkButton(top, fg_color="black",cursor= "arrow", text="", hover_color="black",corner_radius=50, border_width=2, border_color="gray80")
        b.place(relx=0,rely=0, relwidth=1, relheight= 0.5)
        b.configure(image= imagenn)
    ...
    def contacto(self):
        ...
        top.resizable(0,0)
        top.iconbitmap(f"{self.ruta_foto}/icono.ico")
        top.title("Contactanos")
    ...
    def empresa(self):
        ...
        imagenn = CTkImage(Image.open(f"{self.ruta_foto}/Empresa2.png"), size=(650, 325))
    ...
    def buscar_producto(self,*n): 
        if n:  
            if isinstance(n[0], tk.Event): 
                if n[1] == 0:
                    entry = self.entry_buscar.get()
                    produc = self.entry_producto.get().upper()
                    if  n[0] == 1:
                        self.b = "LIVIANA"
                    elif n[0] == 2:
                        self.b = "PESADA"
                        ...
                        self.b = self.entry_linea.get().upper()
                    
                elif n[1] == 4:
                    entry = self.entry_buscar2.get()
                    if entry == "Buscar...":
                        entry = ""
                    produc = ""
                    self.b = self.var.get()
        ...
        self.verificar_si_existe(entry, produc)

    def agrandar(self, event, boton, nombre):
        self.wi = boton.image.width()
        self.hei = boton.image.height()
        resized_auto = Image.open(f"{self.ruta_foto}/{nombre}.png")
    ...
    def crear_tabla(self, dataframe):
            try:
                self.frame_tree.destroy()
            except:pass

            self.frame_tree = tk.Frame(self.frame_segundo, bg="#ededed")
            self.frame_tree.place(relx=0.01, rely=0.11, relheight=0.51,relwidth=0.975)
            ...
            def aplicar_imagen(cod):
                ...
                try:
                    ...
                    self.lab.configure(fg_color="white")
                    imagen_tk = CTkImage(Image.open(f"{self.ruta_foto}/{cod}.jpg"), size=(ancho-25, alto))
                    self.lab.configure(hover_color="white")
                    self.lab.configure(image=imagen_tk)
                    self.lab.image = imagen_tk
                    self.lab.place(relx=0,rely=0,relheight=1,relwidth=1)
                    self.bol = True
                except Exception as e:
                    ancho = self.frame_img.winfo_width()
                    alto = self.frame_img.winfo_height()
                    imagen_tk = CTkImage(Image.open(f"{self.ruta_foto}/nofile.png"), size=(ancho, alto))
                    self.lab.configure(hover_color="#04366D")
                    self.lab.configure(fg_color="#04366D")
                    self.lab.configure(image=imagen_tk)
                    self.lab.image = imagen_tk
                    self.lab.place(relx=0,rely=0,relheight=1,relwidth=1)
                    self.bol = False
            ...
            self.treeview.bind("<<TreeviewSelect>>", lambda event: (on_treeview_select(event), self.table_inf(event)))
            ...
            cant_productos = tk.Label(self.frame_encontra, font=("SEGOE UI", self.letra, "bold"), fg = "gray40", text=f"{len(children)} {texto}", justify=tk.RIGHT, anchor="e", bg="#ededed")
    ...
    def table_inf(self, *event):
            ...
            if len(texto) > 60 and len(texto) <200:
                ...
                partes = [texto[i:i+sexto] for i in range(0, longitud, sexto)]
            ...
            value = [["CÓDIGOS DEL PRODUCTO:",  "LÍNEA:",   "MARCA/MODELO:",        "CLASIFICACIÓN:"],
                    ...
                    [f"OEM: {df[1][0]}",        df[4][0],       texto2]]
 
            self.tabla_inf = t.CTkTable(master = self.frame_inferior, row=4, column=4, values=value, color_phase= True , colors=["white", "white"], header_color="#04366D",font=("SEGOE UI", self.letra+1, "bold"), bg_color= "#ededed")
            self.tabla_inf.edit_row(0 ,text_color="white", font=("SEGOE UI", self.letra+6, "bold"))

            self.tabla_inf.place(relx=0.01, rely=0.05,relwidth=0.98, relheight= 0.9)
            self.tabla_inf.grid_columnconfigure(1, minsize=10)
    ...
    def interfaz_buscador(self, df):
        ...
        def mostrar_foto_grande():
            ...
            item = self.treeview.selection()[0]
            value_1 = self.treeview.item(item, 'values')[0]
            top.title(f"Articulo: {value_1}")
            if self.bol:
                imagen = Image.open(f"{self.ruta_foto}/{value_1}.jpg")
                imagen = imagen.resize((410, 304))
            else:
                imagen =Image.open(f"{self.ruta_foto}/nofile.png")
                imagen = imagen.resize((410, 304))
            ...
            button = ctk.CTkButton(top, text="Descargar imagen",font=("SEGOE UI", self.letra, "bold"), cursor= "hand2", fg_color="#032956", text_color="white", hover_color="dodgerblue3", command=lambda ruta = f"{self.ruta_foto}/{value_1}.jpg", nombre = value_1: guardar_imagen(ruta,nombre),corner_radius=50)
        ...
        self.btn_volver = ctk.CTkButton(self.root, font=("SEGOE UI",  self.letra+5, "bold"), text="Volver", text_color="#04366D", cursor ="hand2", fg_color="white", compound="left",image=self.back, hover_color="gray90", command=self.btn_volver_active, border_color="gray80", border_width=3,corner_radius=50)


        self.btn_volver.place(relx=0.01, rely=0.1)
        self.frame_filtros = tk.Frame(self.frame_segundo, bg="#ededed")
        self.frame_filtros.place(relx=0, rely=0, relheight=0.11,relwidth=1)
        ...
        self.frame_inferior = ctk.CTkFrame(self.frame_segundo, fg_color="#ededed",corner_radius=50)
        ...
        self.lab = ctk.CTkButton(self.frame_img, fg_color="white",cursor= "hand2",command=mostrar_foto_grande, text="", hover_color="white",corner_radius=50, border_width=2, border_color="gray60")
        ...
        def segment(value):   
            self.frame_segundo.focus_set()     
            self.buscar_producto(4)




        self.var = tk.StringVar()
        self.segmentar_lineas = ctk.CTkSegmentedButton(self.frame_filtros, values=[ "TODAS", "LIVIANA", "PESADA", "OTROS MERCADOS"], bg_color="#ededed", fg_color="gray80", text_color="#032956", selected_color="dodgerblue3", font=("SEGOE UI", self.letra+2, "bold"), unselected_color="white", unselected_hover_color="dodgerblue3", command=segment,border_width=3, selected_hover_color="dodgerblue3", variable= self.var)
        self.segmentar_lineas.place(relx= 0.5, rely= 0.6, anchor="center", relheight = 0.45, relwidth=0.25)


        def enfocar(event):
            if self.entry_buscar2.get() == "Buscar...":
                self.entry_buscar2.delete(0,tk.END)
                self.entry_buscar2.configure(text_color='#04366D')

        def verificar_contenido(event):
            try:
                texto = self.entry_buscar2.get()
            except: texto = "-"
            foco = self.frame_segundo.focus_get()
            if foco != self.entry_buscar2 and len(texto) == 0:
                self.entry_buscar2.configure(text_color='gray80')
                self.entry_buscar2.insert(0,"Buscar..." )
            else:
                return

        self.entry_buscar2 = ctk.CTkEntry(self.frame_filtros, width= 200,font=("SEGOE UI", self.diez+2, "bold"),  text_color="gray80", border_width=0,border_color="white", fg_color="white")
        self.entry_buscar2.place(relx= 0.86, rely= 0.6, anchor="center", relwidth= 0.12, relheight = 0.45)
        self.entry_buscar2.insert(0, "Buscar...")
        self.entry_buscar2.bind("<Return>", lambda event, e = 4 :self.buscar_producto(event, e))
        self.entry_buscar2.bind("<FocusIn>", enfocar)
        self.root.bind("<Configure>", verificar_contenido)
        self.crear_tabla(df)


        btn_buscar = ctk.CTkButton(self.frame_filtros, font=("SEGOE UI",  self.letra+5, "bold"), text="",text_color="#04366D", cursor ="hand2", fg_color="white", compound="left", hover_color="gray90",image=self.imagen_lupa, command= lambda: self.buscar_producto(4), border_width=1, border_color = "white", bg_color="white")
        btn_buscar.place(relx=0.9,rely=0.6,relwidth= 0.03,relheight= 0.4,anchor="center")  
        ...
        btn_eliminar = ctk.CTkButton(self.frame_filtros, font=("SEGOE UI",  self.letra, "bold"), text="Limpiar campos", text_color="#04366D", cursor ="hand2", fg_color="#ededed",hover_color="gray80", command=limpiar_datos)   
        btn_eliminar.bind("<Enter>", cambiar_color_enter)
        btn_eliminar.bind("<Leave>", cambiar_color_leave)      
        btn_eliminar.place(relx=0.96,rely=0.6,anchor="center",relwidth= 0.08,relheight=0.45)
    ...
    def verificar_si_existe(self, buscado, produc):
        con = conectar_bd()
        cursor = con.cursor()
        if buscado:
        ...
        if produc:
            if self.b =="":
                consulta = f"SELECT codigo_capemi, descripcion, clasificacion FROM articulos WHERE (clasificacion like '%{produc}%') AND (codigo_capemi LIKE '%{buscado}%' OR descripcion LIKE '%{buscado}%' OR clasificacion LIKE '%{buscado}%' OR codigo_completo LIKE '%{buscado}%' OR marca LIKE '%{buscado}%' OR modelo LIKE '%{buscado}%' OR liviana LIKE '%{buscado}%' OR pesada LIKE '%{buscado}%' OR \"otros mercados\" LIKE '%{buscado}%')"

            else:
                consulta = f"SELECT codigo_capemi, descripcion, clasificacion FROM articulos WHERE (clasificacion LIKE '%{produc}%') AND (liviana LIKE '%{self.b}%' OR pesada LIKE '%{self.b}%' OR \"otros mercados\" LIKE '%{self.b}%')  AND (codigo_capemi LIKE '%{buscado}%' OR descripcion LIKE '%{buscado}%' OR clasificacion LIKE '%{buscado}%' OR codigo_completo LIKE '%{buscado}%' OR marca LIKE '%{buscado}%' OR modelo LIKE '%{buscado}%' OR liviana LIKE '%{buscado}%' OR pesada LIKE '%{buscado}%' OR \"otros mercados\" LIKE '%{buscado}%')"
        else:
            if self.b == "":
                consulta = (f"SELECT codigo_capemi,descripcion,clasificacion FROM articulos WHERE (codigo_capemi LIKE '%{buscado}%' OR descripcion LIKE '%{buscado}%' OR clasificacion LIKE '%{buscado}%' OR codigo_completo LIKE '%{buscado}%' OR marca LIKE '%{buscado}%' OR modelo LIKE '%{buscado}%' OR liviana LIKE '%{buscado}%' OR pesada LIKE '%{buscado}%' OR \"otros mercados\" LIKE '%{buscado}%')")
            else:
                consulta = f"SELECT codigo_capemi, descripcion,clasificacion FROM articulos WHERE (codigo_capemi LIKE '%{buscado}%' OR descripcion LIKE '%{buscado}%' OR clasificacion LIKE '%{buscado}%' OR codigo_completo LIKE '%{buscado}%' OR marca LIKE '%{buscado}%' OR modelo LIKE '%{buscado}%' OR liviana LIKE '%{buscado}%' OR pesada LIKE '%{buscado}%' OR \"otros mercados\" LIKE '%{buscado}%') AND (liviana LIKE '%{self.b}%' OR pesada LIKE '%{self.b}%' OR \"otros mercados\" LIKE '%{self.b}%')"
        ...
        if df.empty:
            if self.b:
                if self.b in "OTROS MERCADOS":
                    def eliminar(event):
                        c.destroy()
                        self.entry_buscar.focus_set()
                    self.root.focus_set()
                    c= CTkMessagebox(title="En Desarrollo", message="PROXIMAMENTE EN NUESTRO CATÁLOGO",icon=f"{self.ruta_foto}/ele_blanco.png", option_1="Gracias", option_focus=1, button_color="#ededed", button_hover_color="dodgerblue3", fg_color="#032956", text_color="white", border_color="dodgerblue3", bg_color="#04366D", title_color="white", button_text_color="#032956", font=("SEGOE UI", 18, "bold"))   
                    c.focus_set()            
                    c.bind("<Return>", eliminar)
                    return
                else:
            ...
            df = df.apply(lambda x: x.str.replace(r'\s+', ' ', regex=True) if x.dtype == 'object' else x)
            try:
                if self.frame_segundo:
                    self.crear_tabla(df)
                else: self.interfaz_buscador(df)
            except:
    ...
    def frame_principal_fn(self):
        ...
        self.frame_publicidad = tk.Frame(self.frame_principal, bg= "#ededed")
        self.frame_publicidad.place(relx=0.5, rely=0.875, relheight=0.25,relwidth=0.6, anchor ="center")
        self.lab = tk.Button(self.frame_publicidad, background= "#ededed", highlightbackground="#ededed", cursor="hand2", highlightcolor="#ededed", highlightthickness=0, borderwidth=0, border=0)
        ...
        label_producto = tk.Label(self.frame_buscador, text = "Clasificación: ", font=("SEGOE UI",  self.letra +2, "bold"), fg= "#04366D",justify="right", bg= "#ededed", anchor="e")
        ...
        self.entry_buscar = ctk.CTkEntry(self.frame_buscador, width= 600,font=("SEGOE UI", self.diez +5, "bold"), height=self.letra+35,  text_color="#04366D",corner_radius=50)
        self.entry_buscar.focus()
        self.frame_buscador.bind("<Return>", lambda event, e = 0 :self.buscar_producto(event, e))
        self.entry_buscar.bind("<Return>", lambda event, e = 0 :self.buscar_producto(event, e))
        self.entry_buscar.place(relx=0.5,rely=0.525,relwidth=0.4,anchor="center")

        producto = obtener_producto()
        self.entry_producto = ctk.CTkComboBox(self.frame_buscador,values=producto ,width= 600,font=("SEGOE UI", self.diez+3, "bold"), height=self.letra+35, button_color="gray70", button_hover_color="#032956",  text_color="white", fg_color="#04366D", dropdown_fg_color="#032956", dropdown_text_color="white", dropdown_hover_color="dodgerblue3",corner_radius=50, cursor="hand2")
        self.entry_producto.set("")
        self.entry_producto.place(relx=0.5,rely=0.65,relwidth=0.4,anchor="center")
        self.entry_producto.bind("<Return>", lambda event, e = 0 :self.buscar_producto(event, e))

        self.entry_linea =ctk.CTkComboBox(self.frame_buscador,values=["LIVIANA", "PESADA", "OTROS MERCADOS"], width= 600,font=("SEGOE UI",  self.diez+3, "bold"), height=self.letra+35, button_color="gray70", button_hover_color="#032956",  text_color="white", fg_color="#04366D", dropdown_fg_color="#032956", dropdown_text_color="white", dropdown_hover_color="dodgerblue3",corner_radius=50)
        self.entry_linea.set("")
        self.entry_linea.place(relx=0.5,rely=0.775,relwidth=0.4,anchor="center")
        self.entry_linea.bind("<Return>", lambda event, e = 0 :self.buscar_producto(event, e))

        btn_buscar = ctk.CTkButton(self.frame_buscador, font=("SEGOE UI",  self.letra+5, "bold"), text="BUSCAR",text_color="#04366D", cursor ="hand2", fg_color="white", compound="left", hover_color="gray90",image=self.imagen_lupa, command= lambda: self.buscar_producto(0), height=self.letra+35,border_color="gray80", border_width=3,corner_radius=50)
        ...
        btn_eliminar = ctk.CTkButton(self.frame_buscador, font=("SEGOE UI",  self.letra, "bold"), text="Limpiar campos", text_color="#04366D", cursor ="hand2", fg_color="#ededed", compound="left",image=self.eliminar, hover_color="gray80", command=limpiar_datos,corner_radius=50)
        ...
        self.liviana.place(relx=0.17,rely=0.225,anchor="center")
        self.pesada.place(relx=0.5,rely=0.225,anchor="center")
        self.otras.place(relx=0.83,rely=0.225,anchor="center")
        ...
        def insertar_imagen(event):
            try:   
                with open(f"{self.ruta_foto}/extension.txt", 'r') as archivo:
                    contenido = archivo.read()
                ancho = self.frame_publicidad.winfo_width()
                alto = self.frame_publicidad.winfo_height()    
                image = Image.open(f"{self.ruta_foto}/banner{contenido}").resize((ancho,alto), Image.BILINEAR)       
                imagen_tk = ImageTk.PhotoImage(image)
                self.lab.configure(image=imagen_tk)
                self.lab.image = imagen_tk
                self.lab.place(relx=0.5,rely=0.5,relheight=1,relwidth=1,anchor="center")
                '''if contenido == ".png":
                    with open(f"{self.ruta_foto}/extension.txt", 'w') as archivo:
                        archivo.write(".jpg")                
                elif contenido == ".jpg":
                    with open(f"{self.ruta_foto}/extension.txt", 'w') as archivo:
                        archivo.write(".png")
        ...
        self.lab.bind("<Button-1>", abrir_enlace)
