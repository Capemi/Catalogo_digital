...
class Ventana_p:
    def __init__(self, root):
        ...
        auto = Image.open("Catalogo_digital-main/logob.png")
        ...
        if monitor.width <= 1280:
            ...
            publi = Image.open("Catalogo_digital-main/banner.png").resize((700, 120))
            ...
            back = Image.open("Catalogo_digital-main/back.png").resize((10, 10))
            ...
            if monitor.width <=1440:
                ...
                alto_ventana = int(monitor.height)
                dev = Image.open("Catalogo_digital-main/dev.png").resize((30,20))
                publi = Image.open("Catalogo_digital-main/banner.png").resize((770, 132))
                lupa = Image.open("Catalogo_digital-main/lupa.png").resize((20, 20))
                ...
                alto_ventana = int(monitor.height)
                dev = Image.open("Catalogo_digital-main/dev.png").resize((30,20))
                publi = Image.open("Catalogo_digital-main/banner.png").resize((945, 162)) 
                ...
                publi = Image.open("Catalogo_digital-main/banner.png").resize((945, 162))
                lupa = Image.open("Catalogo_digital-main/lupa.png").resize((50, 50))
                back = Image.open("Catalogo_digital-main/back.png").resize((100, 100))
                eliminar = Image.open("Catalogo_digital-main/eliminar.png").resize((50, 50))
                ...
                alto_ventana = int(monitor.height)
                dev = Image.open("Catalogo_digital-main/dev.png").resize((40,30))
                publi = Image.open("Catalogo_digital-main/banner.png").resize((1050, 180))
                ...
                alto_ventana = int(monitor.height)
                dev = Image.open("Catalogo_digital-main/dev.png").resize((40,30)) 
                publi = Image.open("Catalogo_digital-main/banner.png").resize((1400, 240))
                lupa = Image.open("Catalogo_digital-main/lupa.png").resize((50, 50))
                back = Image.open("Catalogo_digital-main/back.png").resize((100, 100))
                ...
                alto_ventana = int(monitor.height)
                dev = Image.open("Catalogo_digital-main/dev.png").resize((40,30)) 
                publi = Image.open("Catalogo_digital-main/banner.png").resize((2100, 360))
                back = Image.open("Catalogo_digital-main/back.png").resize((70, 70))
                lupa = Image.open("Catalogo_digital-main/lupa.png").resize((50, 50))
                eliminar = Image.open("Catalogo_digital-main/eliminar.png").resize((50, 50))
        ...
        self.publi = ImageTk.PhotoImage(publi)
        ...
        boton_verificar = tk.Button(self.frame_barra, text = "Verificar actualizaciones", font= ("SEGOE UI", self.diez, "bold"), fg= "#04366D", bg="white", borderwidth=0, highlightthickness=0, cursor="hand2" , activebackground= "white", activeforeground=  "dodgerblue3", command= self.verificar_si_actualizacion)
    ...
    def probar_new_main(self):
        ...
        import sys
        import shutil
        nombre_archivo_principal = os.path.splitext(sys.argv[0])[0]
        def abrir_ventana_nueva():
            # Verificar si existe el archivo principal renombrado
            if os.path.isfile(f"Catalogo_digital-main/{nombre_archivo_principal}.py"):
                # Eliminar la ventana principal
                root.destroy()

                # Ejecutar el archivo principal renombrado en un proceso separado
                os.system(f"python Catalogo_digital-main/{nombre_archivo_principal}.py")
            else:
                # Mostrar un mensaje de error si el archivo renombrado no existe
                messagebox.showerror("Error", f"El archivo '{nombre_archivo_principal}.py' no existe en el directorio actual.")
        ...
        def ejecutar():       
            ...
            if os.path.isfile("Catalogo_digital-main/main2.py"):
                ...
                shutil.move("Catalogo_digital-main/main2.py", f"{nombre_archivo_principal}.py")
                abrir_ventana_nueva()
    ...
    def verificar_si_actualizacion(self):
        ...
        #OBTENER FECHA FOTOS
        def obtener_fecha_fotos():
            def obtener_fecha_modificacion_carpeta(url_repositorio, nombres_archivos):
                # Obtener la información del repositorio utilizando la GitHub API
                api_url = f"https://api.github.com/repos/{url_repositorio.split('/')[3]}/{url_repositorio.split('/')[4]}"
                response = requests.get(api_url)
                repositorio = response.json()
                ...
                # Obtener la fecha de última confirmación (commit) de las carpetas y archivos
                for commit in commits:
                    sha = commit['sha']
                    api_url_archivos = f"{repositorio['url']}/git/trees/{sha}?recursive=1"
                    response_archivos = requests.get(api_url_archivos)
                    archivos = response_archivos.json()

                    # Verificar si las carpetas y archivos están presentes en la lista de archivos
                    carpetas_y_archivos_encontrados = []
                    for archivo in archivos['tree']:
                        if archivo['path'] in nombres_archivos:
                            carpetas_y_archivos_encontrados.append(archivo['path'])
                            fecha_commit = commit['commit']['committer']['date']
                            fecha_commit = datetime.strptime(fecha_commit, "%Y-%m-%dT%H:%M:%SZ")
                            fecha_commit = pytz.utc.localize(fecha_commit).astimezone(pytz.timezone("America/Argentina/Buenos_Aires"))
                            fecha_formateada = fecha_commit.strftime("%Y-%m-%d %H:%M:%S")
                            fecha_dt = datetime.strptime(fecha_formateada, "%Y-%m-%d %H:%M:%S")
                    ...
                    # Verificar si se encontraron todas las carpetas y archivos
                    if set(nombres_archivos) == set(carpetas_y_archivos_encontrados):
                        break
            ...
            nombres_archivos = "fotos"

            # Llamar a la función obtener_fecha_modificacion_carpeta
            fecha = obtener_fecha_modificacion_carpeta(url_repositorio, nombres_archivos)
            return(fecha)
        ...
        def obtener_interna():
            with open("Catalogo_digital-main/verificacion.txt", "r") as file:
                for line in file:
                    date = datetime.strptime(line, "%Y-%m-%d %H:%M:%S")  # Eliminar espacios en blanco y saltos de línea
                    return(date)
        ...
        if diferencia_db.total_seconds() > 0:
            pass
        elif diferencia_db.total_seconds() < 0:
            descargar_repo()
            nueva_fecha_interna(fecha_db)
            self.probar_new_main(self)
        else:
    ...
    def empresa(self):
        ...
        top.geometry(f"500x600+{x}+{y-20}")
        top.resizable(0,0)
        top.iconbitmap("Catalogo_digital-main/icono.ico")
        top.title("Sobre nosotros")
        imagen_pil = Image.open("Catalogo_digital-main/Empresa2.png").resize((500, 250))
        imagenn = ImageTk.PhotoImage(imagen_pil)

        frame = ctk.CTkLabel(top, image=imagenn, text="",).pack()
        
        text = ctk.CTkTextbox(top, font=("SEGOE UI", 14), width=700, height=400, wrap='word', scrollbar_button_color="#032956", cursor= "arrow")
    ...
    def interfaz_buscador(self, df):
        ...
        def on_enter(event):
            self.label_cl = tk.Label(self.frame_img, text="Click para ampliar",  font=("SEGOE UI", self.letra-3, "bold"), bg= "#04366D", fg ="#ededed")
            self.label_cl.place(relx=0.5, rely=0.92, anchor="center")
        def on_leave(event):
            try:
                self.label_cl.destroy()
            except:pass
        ...
        self.frame_inferior = tk.Frame(self.frame_segundo, bg="#ededed")
        self.frame_inferior.place(relx=0, rely=0.7, relheight=0.3,relwidth=1)
        ...
        lab = tk.Button(self.frame_img, background="gray80",cursor= "hand2",command=mostrar_foto_grande)
        ...
        def crear_tabla(dataframe):
            ...
            cant_productos = tk.Label(self.frame_encontra, font=("SEGOE UI", self.letra-3, "bold"), fg = "gray40", text=f"{len(children)} Productos encontrados", justify=tk.RIGHT, anchor="e", bg="#ededed")
            
            cant_productos.place(relx=0,rely=0, relheight=1,relwidth=1)

            label = tk.Label(self.frame_inferior, text = self.b).pack()
        crear_tabla(df)
    ...
    def verificar_si_existe(self, buscado, produc, linea):
        ...
        if df.empty:
            ...
            df = df.applymap(lambda x: re.sub('\s+', ' ', x.strip()) if isinstance(x, str) else x)
    ...
    def frame_principal_fn(self):
        ...
        def resize_botones(event):
            ...
            '''resized_info =self.contactos_copy.resize(valor) 
            resized_info2 = ImageTk.PhotoImage(resized_info)
            self.info.configure(image=resized_info2)
            self.info.image = resized_info2

            resized_lanza =self.lanza_copy.resize(valor) 
            resized_lanza2 = ImageTk.PhotoImage(resized_lanza)
            self.lanza.configure(image=resized_lanza2)
            self.lanza.image = resized_lanza2

            resized_empre =self.empresa_copy.resize(valor) 
            resized_empre2 = ImageTk.PhotoImage(resized_empre)
            self.empre.configure(image=resized_empre2)
            self.empre.image = resized_empre2'''
        ...
        self.entry_producto = ctk.CTkComboBox(self.frame_buscador,values=producto ,width= 600,font=("SEGOE UI", self.diez+3, "bold"), height=self.letra+35,button_color="gray70", button_hover_color="#032956",  text_color="white", fg_color="#04366D", dropdown_fg_color="#032956", state="readonly", dropdown_text_color="white")

        self.entry_producto.set("")
        self.entry_producto.place(relx=0.5,rely=0.65,relwidth=0.4,anchor="center")
   

        self.entry_linea =ctk.CTkComboBox(self.frame_buscador,values=["LIVIANA", "PESADA", "OTROS MERCADOS"], width= 600,font=("SEGOE UI",  self.diez+3, "bold"), height=self.letra+35,button_color="gray70", button_hover_color="#032956",  text_color="white", fg_color="#04366D", dropdown_fg_color="#032956", state="readonly", dropdown_text_color="white")
        self.entry_linea.set("")
        ...
        self.liviana.bind("<Enter>", lambda event, boton= self.liviana, nombre = "logob":self.agrandar(event,boton,nombre))
        self.liviana.bind("<Leave>", lambda event, boton= self.liviana, nombre = "logob":self.achicar(event,boton,nombre))
        ...
        self.frame_publicidad.place(relx=0, rely=0.77, relheight=0.23,relwidth=1) 
        self.label_publi = tk.Label(self.frame_publicidad,image=self.publi, bg= "#ededed")
        self.label_publi.place(relx=0.5,rely=0.5,relheight=1,relwidth=0.6,anchor="center")
