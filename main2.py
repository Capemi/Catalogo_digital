...
class Ventana_p:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("CATÁLOGO CAPEMI ACTUALIZADO - A. Giacomelli SA")
        ...
        boton_verificar = tk.Button(self.frame_barra, text = "Verificar actualizaciones", font= ("SEGOE UI", self.diez, "bold"), fg= "#04366D", bg="white", borderwidth=0, highlightthickness=0, cursor="hand2" , activebackground= "white", activeforeground=  "dodgerblue3", command= self.circulo_carga)
    ...
    def probar_new_main(self):
        ...
        def abrir_ventana_nueva():
            ...
            if os.path.isfile(f"{nombre_archivo_principal}.py"):

                # Eliminar la ventana principal
                self.root.destroy()

                # Ejecutar el archivo principal renombrado en un proceso separado
                os.system(f"{nombre_archivo_principal}.py")
            else:
                # Mostrar un mensaje de error si el archivo renombrado no existe
                tk.messagebox.showerror("Error", f"El archivo '{nombre_archivo_principal}.py' no existe en el directorio actual.")
        ...
        def ejecutar(): 
            ...
            c= CTkMessagebox(title="En Desarrollo", message="CATÁLOGO DIGITAL SE HA ACTUALIZADO CON EXITO",icon="question", option_1="Si", option_2="Ahora no",option_focus=1, button_color="#ededed", button_hover_color="dodgerblue3", fg_color="#032956", text_color="white", border_color="dodgerblue3", bg_color="#04366D", title_color="white", button_text_color="#032956", font=("SEGOE UI", 18, "bold"))   
            c.focus_set()    
            g = c.get()
            if g == "Si":
                abrir_ventana_nueva()        
    ...
    def circulo_carga(self):
        ...
        def iniciar_descarga():
            ...
            x = self.root.winfo_screenwidth() // 2 - 100 // 2
            y = self.root.winfo_screenheight() // 2 - 100 // 2
        
            # Establecer la posición y tamaño de la ventana TopLevelu
            popup.geometry(f"100x100+{x}+{y}")
            popup.title("Descargando...")
            
            # Establece la transparencia de la ventana
            popup.attributes("-alpha", 0.9)  # Ajusta el valor de 0.7 para la transparencia que desees
            
            # Crea el lienzo con el círculo de carga en la ventana emergente
            canvas = tk.Canvas(popup,bg="lightblue")  # Fondo blanco para el círculo de carga
            canvas.pack()
        ...
        def draw_loading_circle(canvas):
            ...
            canvas.create_arc(10, 10, 90, 90, start=self.rotation_degree, extent=90, style=tk.ARC, outline="#04366D", width=15)
            self.rotation_degree -= 10
            if self.rotation_degree >= 360:
                self.rotation_degree = 0
            popup.after(50, lambda: draw_loading_circle(canvas))
